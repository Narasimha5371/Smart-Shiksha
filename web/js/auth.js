/**
 * Smart Shiksha — Authentication module (Auth0 + Dev-mode login).
 *
 * Flow (Auth0 - production):
 *  1. User clicks "Sign in with Google"
 *  2. Auth0 popup → Google account selection
 *  3. Auth0 ID token sent to POST /api/auth/login
 *  4. Backend verifies via JWKS, creates/updates user, returns JWT
 *
 * Flow (Dev-mode - local development):
 *  1. User enters name + email
 *  2. Base64 fake token sent to POST /api/auth/login
 *  3. Backend (DEBUG=true) decodes it, creates/updates user, returns JWT
 *
 * In both cases:
 *  - JWT stored in memory (+ localStorage for persistence)
 *  - All subsequent API calls include Authorization header
 */
const SmartAuth = (() => {
    "use strict";

    // ── Auth0 config ─────────────────────────────
    const AUTH0_DOMAIN    = "dev-vzwcjg03kzp4d44m.us.auth0.com";
    const AUTH0_CLIENT_ID = "ZzN4EGlsBIuOie0tru5LEOZKZh920eMw";
    const API_BASE = "https://smartsiksha.onrender.com/api";

    // ── State ────────────────────────────────────
    let _auth0Client = null;
    let _jwt = null;
    let _user = null;            // backend user object { id, name, email, ... }
    let _onAuthChange = null;    // callback(user|null)

    // ── Public API ───────────────────────────────

    /**
     * Initialise Auth0 client and restore any persisted session.
     * @param {Function} onAuthChange  - called with user object or null
     */
    async function init(onAuthChange) {
        _onAuthChange = onAuthChange;

        // Initialize Auth0 SPA client (loaded via <script> in HTML)
        try {
            if (typeof auth0 !== "undefined") {
                _auth0Client = await auth0.createAuth0Client({
                    domain: AUTH0_DOMAIN,
                    clientId: AUTH0_CLIENT_ID,
                    cacheLocation: "localstorage",
                });
            }
        } catch (err) {
            console.warn("[SmartAuth] Auth0 not available, dev-mode only:", err.message);
        }

        // Try restore JWT from localStorage
        const storedJwt = localStorage.getItem("ss_jwt");
        if (storedJwt) {
            _jwt = storedJwt;
            const user = await _fetchMe();
            if (user) {
                _user = user;
                _notify();
            } else {
                _clearSession();
                _notify();
            }
        } else {
            _notify();
        }
    }

    /**
     * Start Google Sign-In via Auth0 popup.
     */
    async function signInWithGoogle() {
        if (!_auth0Client) throw new Error("Auth0 not available. Use dev-mode login instead.");

        try {
            await _auth0Client.loginWithPopup();

            const claims = await _auth0Client.getIdTokenClaims();
            const idToken = claims.__raw;

            return await _exchangeToken(idToken);
        } catch (err) {
            if (err.message && err.message.includes("Popup closed")) return null;
            console.error("[SmartAuth] Sign-in error:", err);
            throw err;
        }
    }

    /**
     * Dev-mode login with name + email (backend must have DEBUG=true).
     * Creates a base64 fake token, same as the Flutter app's dev flow.
     */
    async function signInWithEmail(name, email) {
        if (!name || !email) throw new Error("Name and email are required");

        // Build a fake token payload (same format as Flutter dev client)
        const payload = JSON.stringify({
            sub: "dev_" + _hashCode(email),
            email: email,
            name: name,
        });
        const fakeToken = btoa(payload);

        return await _exchangeToken(fakeToken);
    }

    /**
     * Sign out and clear local session.
     */
    async function signOut() {
        try {
            if (_auth0Client) {
                await _auth0Client.logout({ logoutParams: { localOnly: true } });
            }
        } catch { /* ignore */ }
        _clearSession();
        _notify();
    }

    function getAuthHeaders() {
        if (!_jwt) return {};
        return { Authorization: `Bearer ${_jwt}` };
    }

    function getToken()    { return _jwt; }
    function getUser()     { return _user; }
    function isSignedIn()  { return !!_jwt && !!_user; }

    // ── Private helpers ──────────────────────────

    async function _exchangeToken(idToken) {
        const resp = await fetch(`${API_BASE}/auth/login`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ id_token: idToken }),
        });

        if (!resp.ok) {
            const err = await resp.json().catch(() => ({}));
            throw new Error(err.detail || "Sign-in failed");
        }

        const data = await resp.json();
        _jwt = data.access_token;
        _user = data.user;
        localStorage.setItem("ss_jwt", _jwt);
        localStorage.setItem("ss_user_id", _user.id);
        _notify();
        return _user;
    }

    async function _fetchMe() {
        try {
            const resp = await fetch(`${API_BASE}/auth/me`, {
                headers: { Authorization: `Bearer ${_jwt}` },
            });
            if (!resp.ok) return null;
            return await resp.json();
        } catch {
            return null;
        }
    }

    function _clearSession() {
        _jwt = null;
        _user = null;
        localStorage.removeItem("ss_jwt");
        localStorage.removeItem("ss_user_id");
    }

    function _notify() {
        if (typeof _onAuthChange === "function") {
            _onAuthChange(_user);
        }
    }

    /** Simple string hash (same as Dart's String.hashCode logic) */
    function _hashCode(str) {
        let hash = 0;
        for (let i = 0; i < str.length; i++) {
            hash = ((hash << 5) - hash) + str.charCodeAt(i);
            hash |= 0;
        }
        return Math.abs(hash);
    }

    // ── Expose ───────────────────────────────────
    return {
        init,
        signInWithGoogle,
        signInWithEmail,
        signOut,
        getAuthHeaders,
        getToken,
        getUser,
        isSignedIn,
    };
})();
